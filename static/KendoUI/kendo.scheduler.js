...
var ScheduleMeeting="";
var NewLesson="";
if (localStorage.getItem("Language") && localStorage.getItem("Language") == "cn") {
    AddLesson = "新建";
    Search = "搜索...";
    todayLanguage="今天";
    dayTime="日";
...
(function($, undefined) {
    var kendo = window.kendo,
        ...
        TOOLBARTEMPLATE = kendo.template('<div class="k-floatwrap k-header k-scheduler-toolbar">' +
          ...
          '#}#' +
             ...
             // [Change -- add search box]
            '<ul class="k-reset k-header" style="float: left; margin-left: 1em;">' +
                '<li>' +
            '<span class="k-textbox k-space-right" style="width: 200px; border-width: 0; background: white;" > <input type="text" placeholder="' + Search+'" id="scheduler-search-input"/> <a href="\\#" class="k-icon k-i-search" id="scheduler-search-icon"></a> </span>' +
                '</li>' +
            '</ul>' +

             // [Change -- add filter icon]
            '<ul class="k-reset k-header" style="float: left; margin-left: 1em;">' +
                '<li class="k-state-default k-state-selected" id="scheduler-filter" style="display:none;"><a role="button" title="Filter" href="\\#" class="k-link" style="padding:1px 6px; color:white;"><span style="font-size:20px;" class="icon icon-filter"></span></a></li>' +
            '</ul>' +

            // [Change -- add a button to create event]
            '<ul class="k-reset k-header" style="float: right; margin-right: 1em;position:relative;">' +
            '<li class="k-state-default k-state-selected" id="scheduler-add-lesson"><a role="button" href="\\#" class="k-link" style="color:white;"><i class="ivu-icon ivu-icon-plus-round" style="margin-right: 0.5em;"></i> <span>' + AddLesson+'</span></a></li>' +
            // [Change -- add new menu list]
            '<div id="scheduler-MenuList" class="scheduler-Menu-List">'+
            '<ul><li id="schedule-do-metting-now">' + MettingNow+'</li><li id="schedule-metting">' + ScheduleMeeting+'</li><li id="schedule-new-lesson">' + NewLesson+'</li></ul>'+
            '</div>'+
            '</ul>' +
    ...
    */
    ...
    var Scheduler = DataBoundWidget.extend({
        ...
        _keydown: function(e) {
            // [Change --- ignore keydown event from search input]
            if (e.target.id === 'scheduler-search-input')
              return;
