...
export default {
  ...
  methods: {
    setShowChat() {
      ...
      let phoneFlag = name!=="document"&& name!=="attachment" && name!=="sync"&& name!=="play" //手机不加载的模块
      if(phoneFlag){
        this.showChat2 = true;
        this.$nextTick(()=>{
          this.showChat = name !== 'login' && name !== 'register' && name !== 'join' && name !=='forgetPassworld'&& name!="kloudsync-invitation-notice"
