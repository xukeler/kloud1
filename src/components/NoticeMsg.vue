  ...
  export default {
    ...
    methods: {
      ...
      init()
      {
        ...
        websocket.setsendmessagecallback((d) =>
        {
          ...
          if (obj.actionType == 1 || obj.actionType == 3)//start lesson
          {
            ...
            //if (location.href.toLowerCase().indexOf("#/live?incidentid=")==-1)
            {
              //setTimeout(() =>
              //{
              //  this.$router.push('/live?incidentid=' + obj.meetingID);
              //  setTimeout(() =>
              //  {
              //    this.changeAttachment({ attachmentUrl: obj.attachmentUrl, attachmentID:"" });
              //  }, 2000);
              //}, 100);
        ...
        {
          //if (d.lastActiveMeetingId != "" && d.lastActiveMeetingId != "0" && location.href != "#/live?incidentid=" + d.lastActiveMeetingId)
          //{
          //  this.$router.push('/live?incidentid=' + d.lastActiveMeetingId);
          //}
      ...
      lessonOK()
      {
        ...
        if (location.href.toLowerCase().indexOf("/live?id=" + this.messageInfo.meetingID) == -1 || this.$router.currentRoute.name!="live")
        {
          ...
          {
            //this.$router.push({ name: 'live', query: { incidentid: this.messageInfo.meetingID, type: 1 } });
            //this.$router.go('/live?id=' + this.messageInfo.meetingID);
            //if (location.href.toLowerCase().indexOf("/live?id=") == -1)
            {
              this.$router.push('/live?id=' + this.messageInfo.meetingID);
            }
            //else
            //{
            //  window.location.href = '/live?id=' + this.messageInfo.meetingID;
            //}

          }
          else
          {
            //this.$router.push('/live?incidentid=' + this.messageInfo.meetingID);
            //this.$router.push('/live?id=' + this.messageInfo.meetingID);
            //this.$router.go('/live?id=' + this.messageInfo.meetingID);
