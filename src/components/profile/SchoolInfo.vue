<template>
  <div id="user-profile-school">
    <div class="preview-part">
      <Button type="info" size="large" @click="handlePreview">Preview</Button>
    </div>

    <div class="status-part">
      <div class="part-header">Status</div>
      <div style="color:#2b85e4">Application submitted, Under review.  <Button type="primary" @click="handleStatusChange">Withdraw application</Button></div>
    </div>

    <div class="info-part">
      <school-info-block :school="school" ref="schoolInfo"></school-info-block>
    </div>

    <div class="introduce-part">
      <div class="part-header">Slogan</div>
      <Input v-model="slogan" type="textarea" :autosize="{minRows: 3, maxRows: 10}"></Input>
    </div>

    <div class="introduce-part">
      <div class="part-header">About our school</div>
      <textarea id="schoolDesc"  v-model="introduction" style="height:400px;" placeholder="some description about this course"></textarea>
    </div>

    <div class="interests-part">
      <div class="part-header">Teaching subjects
        <div class="part-action-button">
          <Button type="text" icon="plus-round" @click="handleAddInterest">Add subject</Button>
        </div>
      </div>
    ...
    <div class="picture-part">
      <div class="part-header">Front page picture</div>
        ...
        <div class="school-picture" v-if="picture.file">
          ...
          <div class="school-picture-cover">
            <div style="padding-top: 100px">
              ...
              <p>Click or drag picture here to change</p>
            </div>
          </div>
        </div>
    ...
    <div>
      <Modal v-model="modalPreview"  class-name="school-preview" title="School Preview" :width="90" :mask-closable="false" :styles="{top: '1em'}">
        <div>
      ...
      <Modal v-model="modalInterest" :mask-closable="false" :width="640" title="Add Teaching Subject">
        ...
        <div slot="footer">
          <Button type="primary" @click="handleEditingInterestOK" :loading="loading">OK</Button>
          <Button @click="modalInterest = false">Cancel</Button>
        </div>
      </Modal>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      readData() {
        ...
        this._getData(url, null, (result) => {
          if (self.schoolId == auth.getCustomerID()) {
            self.school.name = "My school (me as an independent tutor)";
          }else{
      ...
      handleStatusChange() {
        ...
        this.$Modal.confirm({
          title: "Close",
          content: "Do you want to withdraw your application?",
          okText: "Yes",
          cancelText: "No",
          onOk: () => {
