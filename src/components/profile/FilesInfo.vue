<template>
  <div id="user-profile-files">
    <div class="part-header">
      <span class="filter-text">{{$t('SelectTags.TagFilter')}}</span>
      <div class="select-tag-input" ref="dropdownSelected">
        <Dropdown trigger="custom" style="width:100%" :visible="visible">
          <div class="input-space" @click="showTag">
            <span
              v-show="!seletedTagArray"
              class="input-placeholder"
            >{{$t('SelectTags.PlaceholderTagFilter')}}</span>
            <div id="chooseContent" class="choose-content" v-show="seletedTagArray">
              <div
                class="tags-item"
                style="margin-left:15px;"
                v-for="(item,itemIndex) in selectedModel"
                :key="itemIndex"
                :style="{backgroundColor:colorSelectNumber(item.ColorID)}"
              >
                <div
                  class="left-triangle"
                  :style="{borderRightColor:colorSelectNumber(item.ColorID)}"
                ></div>
                <p>{{item.TagName}}</p>
              </div>
            </div>
            <div class="arrow-down-icon">
              <!-- <span class="tagEllipsis" >...</span> -->
              <Icon
          ...
          <DropdownMenu slot="list" id="checkIconParent">
            <ul class="dropdown">
              <li>
                <span>{{$t('SelectTags.Tags')}}({{tagsLength}})</span>
                <span style="float:right;padding:2px;text-decoration:underline" @click="tagEdit">
                  <a>{{$t('SelectTags.Manage')}}</a>
                </span>
              </li>
              <div class="dropdown-content">
                <li v-for="(item,index) in tagsList" :key="index" @click="addTag(item,index)">
                  ...
                  <div class="selected-this-tag" :id="'kloudsync-filter-selected-tag'+index">
      ...
      <div
        v-show="!uploadingInfo.ready"
        :title="uploadingInfo.name"
        style="width:100%;padding:3px; position:relative;height:26px; margin:10px 0px;"
      >
        <span
          style="width:80%;white-space: nowrap;overflow: hidden;text-overflow: ellipsis; display:block; position:absolute;z-index:2;"
        >{{uploadingInfo.name}}</span>
        <div
        ...
        >
          <div
            ...
            :style="'width:'+uploadingInfo.process+'%'"
          ></div>
          <div
            style="position: absolute;right: 0px;top: 0px;padding:3px 3px 0px 0px;"
          >{{uploadingInfo.status}}&nbsp;{{uploadingInfo.process}}%</div>
        </div>
      </div>
      ...
      >
        <Form :model="editingFile" label-position="right" :label-width="120">
          ...
          <FormItem :label="$t('EditFileWithTag.Tag')">
            <Edit-file-select-tags
              ref="EditFileSelectTags"
              @editSelectTags="getEditSelectTags"
              :editTagList="editingFile.tagList"
            ></Edit-file-select-tags>
          </FormItem>
    ...
    >
      ...
      <div
        class="chooseNewFileAndTag"
        style="background: #fff;border: 1px dashed #dddee1;border-radius: 4px;"
        v-show="!filesShow"
      >
        <div class="fileIconParent">
          <div class="fileIconShow">
            ...
            <p
              style="overflow: hidden; max-width:100px;text-overflow: ellipsis;white-space: nowrap;"
            >{{filesObj.fileName}}</p>
          </div>
          <div class="deleteFileIcon">
            <Icon
              type="ios-trash-outline"
              :title="$t('AssignmentList.DeleteFile')"
              size="26"
      ...
      <div slot="footer">
        ...
        <Button
          type="primary"
          id="btnFilesInfoUploadFileOK"
          @click="upLoadfileWithTagOk"
        >{{$t('Base.OK')}}</Button>
      </div>
    </Modal>
    <ManageTags :showAddTagsflagParent="showAddTagsflag" ref="manageShow" :teamId="teamId.default"></ManageTags>
  </div>
</template>
...
import a from "../../../static/KendoUI/js/jszip.min";
import ManageTags from "../kloudsync/ManageTags";
...
export default {
                ...
                tagArrayList.map((item, index) => {
                  ...
                  return h(
                    "div",
                    {
                      class: "tags-item",
                      style: {
                        backgroundColor: this.colorSelectNumber(item.ColorID),
                        marginLeft: left
                      }
                    },
                    [
                      h("div", {
                        class: "left-triangle",
                        style: {
                          borderRightColor: this.colorSelectNumber(item.ColorID)
                        }
                      }),
                      h(
                        "span",
            ...
            if (false) {
              return h("div", [
                ...
                h(
                  ...
                  {
                    class: "table-action-icon",
                    attrs: { title: "Copy to School Files" },
                    on: {
                      click: () => {
  ...
  created() {
    document.body.addEventListener("click", this.closeTag);
  },
  mounted() {
    this.initialize();
  ...
  beforeDestroy() {
    document.body.removeEventListener("click", this.closeTag);
  ...
  methods: {
    closeTag(envent) {
      if (!this.$refs.dropdownSelected.contains(envent.target)) {
        this.visible = false;
      }
    },
    initial() {
    ...
    tagEdit() {
      ...
      this.$nextTick(() => {
        this.$refs.manageShow.initial();
      });
    },
    addTag(item, ind, Event) {
      let taget = event.currentTarget;
      if (
        $(taget)
          .children(".selected-this-tag")
          .css("display") == "none"
      ) {
        $(taget)
          .children(".selected-this-tag")
        ...
        this.$nextTick(() => {
          ...
          _this._getData(
            ...
            result => {
              ...
              _this.tagsListCopy = result;
              _this.tagsList = result;
              _this.tagsLength = _this.tagsList.length;
              _this.$nextTick(() => {
                if (_this.selectedModel.length == 0) {
                  return;
                }
                let array = _this.selectedModel.map(item => {
                  return item.TagID;
                });
                _this.tagsList.forEach((item, index) => {
                  if (array.includes(item.TagID)) {
                    $("#kloudsync-filter-selected-tag" + index).css(
                      "display",
                      "block"
                    );
                  }
    ...
    updateFileTag(fileId) {
      ...
      let url =
        GetAPIUrl() +
        "AttachmentTag/UpdateTags?itemID=" +
        fileId +
        "&tagIDs=" +
        self.selectEditTagsStr +
        "&type=" +
        self.uploadType;
      self._postData(
        url,
        ...
        webapi.UploadFile2FavWithHash(-1, -1, filename, hash).then(result => {
          if (result.RetCode == 0) {
            ...
            self.$refs.uploadfile.Upload(
              input.files[0],
              GetAPIUrl() +
                "FavoriteAttachment/AddNewFavoriteMultipart?Description=description&Hash=" +
                hash,
              resultData => {
    ...
    async uploadVideoInputChange() {
      ...
      if (result.RetCode == 0) {
        ...
        var name =
          "tempupload/" +
          util.GetMD5String(
            auth.getUserIDEx() + "_" + filename + "_" + new Date().toString()
          );
        ...
        self.$refs.uploadfile.UploadCloud(
          input.files[0],
          name,
          async (guid, convertParam) => {
            if (type == "") {
              var addresult = await this.uploadNewFile(
                filename,
                servername,
                result.RetData.FileID,
                1,
                hash,
                input.files[0].size
              );
            ...
            this.uploadingInfo.process = 0;
            this.uploadingInfo.status = this.$t("Common.Converting");
            await webapi.startConverting({
              Key: name,
              DocumentType: type,
              Bucket: convertParam,
              TargetFolderKey: result.RetData.Path
            });
            //console.log("token:"+token);
            util.setIntervalEx(async () => {
              //loger.log("convertTimer");
              var cresult = await self.updateProcessNew(guid, {
                Key: name,
                Bucket: convertParam
              });
              //console.log(result);
    ...
    async uploadNewFile(filename, servername, fileid, pagecount, md5, size) {
      ...
      newfile.FileName =
        servername.lastIndexOf(".") > -1
          ? servername.substr(0, servername.lastIndexOf("."))
          : servername;
    ...
    async updateProcessNew(guid, token) {
      ...
      if (res.Success == true) {
        ...
        if (res.Data.CurrentStatus == 0) {
          //0: Preparing, 1: Converting, 2: wait to send, 3: Failed 4:send  5:done
          //file.process+=1;
    ...
    readData() {
      ...
      if (this.searchFile) {
        url +=
          "&searchText=" +
          encodeURIComponent(base64encode(utf16to8(trimStr(this.searchFile))));
      }
    ...
    handleEditFileOK() {
      ...
      if (this.filesId != 0) {
        let url =
          GetAPIUrl() +
          "FavoriteAttachment/RenameAttachment?ItemID=" +
          this.filesId +
          "&title=" +
          editTitle;
        this._postData(
    ...
    handleViewFile(params) {
      ...
      // return;
      webapi
        .AddTempLessonWithOriginalDoc(
          params.row.detail.AttachmentID,
          params.row.detail.Title
        )
        .then(res => {
          //res.RetData
          this.$router.push({
            name: "docview",
            params: { id: res.RetData.ItemID }
          });
        });
    },
    handleShareFile(params) {
      //console.log(params.row.detail);
      if (params.row.detail.FileType == 5 || params.row.detail.FileType == 4) {
        this.$Message.error("Can't share this file!");
        return;
      }
    ...
    handleMoveFile(params) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          let url =
            GetAPIUrl() +
            "FavoriteAttachment/CopyAttachmentsBetweenTeacherAndSchool?attachmentIDs=" +
            params.row.id +
            "&schoolID=" +
            auth.getSchoolID();
    ...
    handleDeleteFile(params) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          let url =
            GetAPIUrl() +
            "FavoriteAttachment/RemoveFavorite?itemIDs=" +
            params.row.id;
