<template>
  <div class="register-page">
    <div :class="isOrganizationPage?'kloudsync-register-background':'register-background'" ></div>

    <div class="register-content" v-show="!isOrganizationPage">
      <div>
        ...
        <div>
          <Modal v-model="selectCountry.modal" :title="$t('Register.PleaseSelectCountry')" :mask-closable="false" :width="600" class-name="register-select-country-modal">
            ...
            <Input v-model="selectCountry.searchText" :placeholder="$t('Register.PlaceholderSearchCountry')" :icon='selectCountry.searchIcon' @on-change="handleSelectCountrySearch" @on-click="handleSelectCountrySearchCancel" style="width: 50%; margin-bottom: 1em;"></Input>
    ...
    <kloudsync-register-part :is-organization-page="isOrganizationPage" ref="kloudsyncRegisterPart"></kloudsync-register-part>
  </div>
</template>
...
<script>
  ...
  import KloudsyncRegisterPart from "./kloudsync/KloudsyncRegisterPart.vue";
    ...
    methods: {
      ...
      registerUser(code){
        ...
        if (this.$route.query.meeting) {
          redirect = {name: 'live', query: {id: '-' + this.$route.query.meeting}};
        }
        ...
        authenticator.register(this, credentials, redirect, (result) => {
          if (result) {
            ...
            if(IsKloudSync()){
              this.isOrganizationPage = true;
              this.$nextTick(()=>{
                this.$refs.kloudsyncRegisterPart.loadCompanies();
              })
            }
          }
        });
