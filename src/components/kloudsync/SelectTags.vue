<template>
    <div class="tag-filter">
      <span class="tag-text">{{$t('SelectTags.TagFilter')}}</span>
      <div class="select-tag-input" ref="dropdownSelected">
        <Dropdown :trigger="dropdownType" style="width:100%">
          <div class="input-space" @click="showSelectHande">
            <span v-show="!seletedTagArray" class="input-placeholder">{{$t('SelectTags.PlaceholderTagFilter')}}</span>
            ...
            <div class="arrow-down-icon">
              ...
              <Icon :class="{rotateUp:showSelectTagList,rotateDown:!showSelectTagList}" type="arrow-down-b" style="color:black;" size="14"></Icon>
            </div>
          </div>
          <DropdownMenu slot="list" id="checkIconParent">
            <ul class="dropdown">
              <li>
                ...
                <span style="float:right;padding:2px;text-decoration:underline" @mousedown.stop="manageTags()"><a>{{$t('SelectTags.Manage')}}</a></span>
              </li>
      ...
      <manage-tags :showAddTagsflagParent="showAddTagsflag" ref="manageTagInit" :teamId="teamId"></manage-tags>
    </div>
</template>
...
export default {
  ...
  created () {
    document.body.addEventListener('click',this.rotateSelectTagIcon);
  },
  mounted() {
    this.$emit("filterSelectTags",this.selectedModel);
  },
  beforeDestroy () {
     document.body.removeEventListener('click',this.rotateSelectTagIcon);
  },
  methods: {
    rotateSelectTagIcon(envent){
      if(!this.$refs.dropdownSelected.contains(envent.target)){
        this.showSelectTagList=false;
      }
    },
    //颜色枚举
    ...
    clearChooseTag(){
      this.selectedModel = [];
      $("#checkIconParent .selected-this-tag").css("display","none");
    },
    refreshTagList(){
      let url=GetAPIUrl()+"Tag/List";

      if(this.teamId != -1){//team
    ...
    afterManageDeletOrReplace(){
      ...
      for(let index=0;index<self.selectedModel.length;index++){
        let flag = false;
        let replaceItem = {};
        let currentItem = self.selectedModel[index].TagID;
        for(let i of self.tagsList){
          if( currentItem == i.TagID){
            flag = true;
            replaceItem = i;
            break;
          }
      ...
      self.$nextTick(()=>{
         ...
         self.$emit("filterSelectTags",self.selectedModel);
         self.$parent.selectedRefresh();
      })
    },
    manageTags(){
    ...
    showSelectHande(){
      this.showSelectTagList=!this.showSelectTagList;
      this.dropdownType='click';

      if(!this.showSelectTagList){
        return
      }
      ...
      this.$nextTick(()=>{
        let url=GetAPIUrl()+"Tag/List";
        if(self.teamId != -1){//team
          url += "?TeamID="+self.teamId;
        }
        
        self._getData(url,null,(result)=>{
          self.tagsList=result;
          self.tagsLength=self.tagsList.length;
          ...
          self.$nextTick(()=>{
            ...
            self.tagsList.forEach((item,index)=>{
              if(array.includes(item.TagID)){
                $("#kloudsync-filter-selected-tag"+index).css("display","block");
              }
            });
          })
        },null)
    ...
    //是否显示省略号
    isShowtagEllipsis(){
      let width=parseInt($("#chooseContent").css("width"));
      if(width>=210){
        return true;
      }
      return false;
