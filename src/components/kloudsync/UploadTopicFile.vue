<template>
    <div>
        ...
        <Modal
            v-model="selectFav"
            title="Select document add to space"
            width="800"
            :closable="false"
            :mask-closable="false"
            @on-ok="selectFavOK"
        ...
        >
            <div slot="header" style=" height:30px;">
                ...
                >Select document add to space</span>
            ...
            <div id="topicFavList" style="height:300px; overflow:auto;">
        ...
        >
            ...
            <div style="padding: 0px 0px 10px;">
                    <Select v-model="selectSpace_Team" style="width:200px" @on-change="onSelectSpace_TeamChange($event);">
                    <Option v-for="item in selectSpace_TeamList" :value="item.id" :key="item.id">{{ item.name }}</Option>
                </Select>
                <Select v-model="selectSpace_Space" style="width:200px" @on-change="onSelectSpace_SpaceChange($event);">
...
export default {
  ...
  methods: {
    ...
    showFavList(){
      webapi.getMyFavorite(1, -1).then(d => {
            this.favList = [];
    ...
    selectSpaceOK(){
        ...
        webapi.addSpace2Topic(topicID, this.selectedSpaceAttachment).then(
            () => {
    ...
    onSelectSpace_SpaceChange(val,callback){
        ...
        webapi.GetSpaceAttachmentList(val,0,1000).then(res=>{
            //console.log(res);
