...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    next(vm=>{
       vm.changeShowcacheComponentList(['kloudsync-meeting']);
    });
  },
  beforeRouteLeave(to, from, next){
    if(to && to.name && (to.name==="live" || to.name==="kloudsync-schedule-meeting")){
      this.changeShowcacheComponentList(['kloudsync-meeting']);
    }else{
  ...
  methods: {
    ...
    ...mapMutations(["changeLessonID","changeShowcacheComponentList"])
  }
};
</script>
