...
<div class="sync-room-left-part">
        ...
        <span class="team-item-left">
          <Poptip ref="poptipTeam" placement="bottom-start">
            ...
            <div slot="title" style="min-height: 32px">
              ...
              <Button shape="circle" style="float:right" @click="handleAddTeam">{{$t('KloudSyncDocLeftPart.AddNewTeam')}}</Button>
            </div>
            <div slot="content" >
              ...
              <div class="team-list" :class="teamListClass">
  ...
  <div class="space">
    <div class="space-title">
      ...
      <span v-if="isTeamOwnerAdmin" style="padding:15px;cursor:pointer;" :title="$t('KloudSyncDocLeftPart.CreataNewSpace')" @click="handleAddSpace" v-show="kloudsyncTopic.currentTeam && kloudsyncTopic.currentTeam.id > 0">
    ...
    <div class="space-content">
      <!-- <div class="space-item" style="color:#2d8cf0;" @click="handleAddSpace" v-show="kloudsyncTopic.currentTeam && kloudsyncTopic.currentTeam.id > 0">
  ...
  <Modal v-model="spaceOrteamProperty" :mask-closable="false" :title="spaceOrteamPropertyTitle" width="600">
    <div class="doc-space-or-team-modal-property">
      ...
      <div class="item-middle-title">
        <div style="color: #bdc5b8;flex:1 1 180px">{{typeMember}}</div>
        ...
        <div style="color: #2b85e4;cursor: pointer;">
          <Dropdown placement="bottom" @on-click="handleAddMemberAction">
            <div style="display:flex;align-items:center;justify-content: center;" v-if="!isTeam && isSpaceOwnerAdmin">
              <span style="margin-left: 10px;display:inline-block">{{$t('KloudSyncDocLeftPart.InviteNew')}}</span>
              <Icon style="padding-left: 5px;" type="ios-plus" size="32"></Icon>
            </div>
            <div style="display:flex;align-items:center;justify-content: center;" v-if="isTeam && isTeamOwnerAdmin">
              <span style="margin-left: 10px;display:inline-block">{{$t('KloudSyncDocLeftPart.AddTeamAdmin')}}</span>
              <Icon style="padding-left: 5px;" type="ios-plus" size="32"></Icon>
            </div>
            <DropdownMenu slot="list">
      ...
      <div class="item-members item-members-adminOrOwner" v-show="temAdminOrOwnerist.length!=0">
        <div v-for="(item,index) in temAdminOrOwnerist" :key="index" class="member-item">
          ...
          <Dropdown placement="bottom" class="more-action">
  ...
  <Modal v-model="modalAddTopic" :mask-closable="false" :title="$t('KloudSyncDocLeftPart.CreateNewSyncRoom')" width="700">
    ...
    <div class="add-syncroom-modal" v-show="openTeam && !openSpace">
    ...
    <div class="add-syncroom-modal" v-show="!openTeam && openSpace">
      <div class="team-item" @click="handleAddSpace()">
  ...
  methods: {
    ...
    handleInviteOK() {
      ...
      let data = {
        ...
        InviteToType:this.isTeam?3:1,//space
    ...
    showSpaceOrTeamPropertyModal(str){
      ...
      if(str=="team"){
        ...
        this.spaceOrTeamName = this.kloudsyncTopic.currentTeam.name;
      }else{
        ...
        this.spaceOrTeamName = this.kloudsyncTopic.currentSpace.name;
      }
      ...
      if(self.teamparam && parseInt(self.teamparam.teamId)!=-1){//当只保存机构的时候，会传一个teamId值为-1，故此加限定条件。
        for(let item of self.teamList){
          if(parseInt(self.teamparam.teamId)==parseInt(item.id)){
            ...
            if(self.$refs.poptipTeam){
              self.$refs.poptipTeam.ok();
            }
            self.$nextTick(()=>{
