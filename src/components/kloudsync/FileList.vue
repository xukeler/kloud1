  ...
  methods: {
    ...
    handMoveOk(){
      if(this.moveFile.selecetTeamId===-1){
        this.$Message.info({content: this.$t('KloudSyncDocuments.PleaseSelectTeam'),duration: 3});
        return;
      }else if(this.moveFile.selecetSpaceId===0){
        this.$Message.info({content: this.$t('KloudSyncDocuments.PleaseSelectASpace'),duration: 3});
        return;
      }
    ...
    handleMovetoFile(params){
      ...
      if(params){
        ...
        if(selection.length ==0){
          this.$Message.warning({content: this.$t('KloudSyncDocuments.PleaseSelectDoc'),duration: 3});
          return;
        }
      }
    ...
    handleViewFile(params){
      ...
      {
          this.$Message.error(this.$t('KloudSyncDocuments.CantView'));
          return;
      }
      var url=location.origin.toLowerCase();
