                    ...
                    <section id="livePanelTopCtl" style="display: flex;justify-content: center;">
                            ...
                            <div id="liveFileList" class="livefilelist" slot="content">
                                ...
                                >
                                    <label style="height:100%;width:100%; display:inline-block;">
                                        ...
                                        <div v-if="IsKloudSync" style="position: absolute;top: 2px;right: 5px;cursor:pointer;" @click.stop="AddFromCamera(1,$event);"><Icon size="22" type="camera"></Icon></div>
                                    </label>
                                </div>
                                <Button
                                    type="primary"
                                ...
                                >{{IsKloudSync?$t("Live.UploadFromSpace"):$t("Live.TakePhoto")}}</Button>
                                <Progress
                        ...
                        >
                            ...
                            <div id="liveSetting" style="width:300px;" slot="content">
                                <control-setting ref="controlsetting"></control-setting>
                            </div>
                        </Poptip>
                        <Tooltip
                                :content="$t('TeacherInfo.QuitTitle')"
                                placement="top"
                                transfer
                            >
...
export default {
    ...
    created: function() {
        ...
        if (!auth.checkAuth()) {
            this.showGlobalBlockMsg("", false);
            var url = "/live?id=" + this.getIDFromUrl();
    ...
    methods: {
        ...
        showGlobalBlockMsg(msg, show = true) {
            if (show == true) {
                if ($("#GlobalBlockMsg").length > 0) {
                    ...
                    var html =
                        "<div id='GlobalBlockMsg' style='position:absolute; top:0px; left:0px;z-index:50002; width:100%; height:" +
                        hei +
                        "px;line-height:" +
                        hei +
        ...
        invitelinkCopy() {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                ...
                if(!url || url=="")
                {
                    url=location.href.toLowerCase().replace("/live?id=", "/join/");
                }
        ...
        WelMsgViewLesson(item) {
            ...
            //this.$router.push({ name: 'live', query: { incidentid: item.id } });
            this.$router.push("/live?id=" + item.id);
            //this.$router.replace('/live?id=' + item.id);
            //this.mounted();
            // setTimeout(() =>
            // {
            //   //this.$router.go({ path: '/live', query: { incidentid: item.id } });
            //   this.$router.go('/live?id=' + item.id);
            // }, 20);

            //this.$router.go({ path: '/live', query: { incidentid: item.id } });
            //this.$router.push('/live?incidentid=' + item.id);
            //this.$router.replace({
            //  path: '/live',
            //  query: {
            //    'incidentid': item.id
        ...
        ShareDocument(powerChange = false) {
            if(Role==3)
            {
                this.$Message.error(this.$t("Live.NoPermission"));
                return;
            }
        ...
        userListMenuClick(name, userid) {
            ...
            {
                if (GetMeetingCount() >= 7) {
                    this.$Message.error("Too many student!");
                    return;
                }
                UptoStudent(
                    selectuser.attr("id").replace("user-", ""),
        ...
        uploadFileInputChange() {
            ...
            if (this.$refs.uploadfile.IsUploading()) {
                this.$Message.error(this.$t("Live.WaitForFileUploading"));
            ...
            if (input.files[0].size >= 1024 * 1024 * 100) {
                this.$Message.error(this.$t("Live.FileTooBig"));
                return;
            }
            var accept = [
                ".jpg",
            ...
            ext = ext.toLowerCase();
            if (!IsExistInArr(accept, ext)) {
                this.$Message.error(this.$t("Live.FileNotSupport"));
                return;
            }
