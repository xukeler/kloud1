<template>
  <div id="courseLecture" class="course-lecture">
    <div id="previewHolderOuter" class="previewHolderOuter">
      ...
      <div v-show="headTitleShow" id="liveLessonStat" @dblclick="dblClickTip($event);" style="position:absolute; left:20px; top:40px; z-index:114; margin:3px 0px 0px 3px;background-color:#2d8cf0;padding:10px;border-radius:6px;color:white;">
        <span style="border-radius: 100%;padding: 2px;display:inline-block;width:20px; height:20px;text-align:center;line-height:15px;vertical-align:middle;">
            <Icon type="calendar" size="12" style="color:white;"></Icon>
        </span>
        <span style="font-weight:bold;">{{headCourseTitle}}</span>
        <span>{{course.Title}}</span>
        <span style="margin:auto 10px">></span>   
        <span style="font-weight:bold;">{{headTitle}}</span>
          ...
          <section id="livePanelTopCtl" style="display: flex;justify-content: center;">
              ...
              >
                  ...
                  <div id="liveMoreAction" style="" slot="content">
                      <div id="liveThink" style="width:350px;">
                          <div style="color:#2db7f5; text-align:left; font-size:18px; border-bottom:1px solid #ccc;width:350px; overflow:hidden;text-overflow: ellipsis;">{{attachmentName}}</div>
            ...
            <Poptip placement="top"  @on-popper-show="disableMoreToolTip=true" @on-popper-hide="disableMoreToolTip=false">
                ...
                <div id="previewLessonMoreActions" slot="content">
                    ...
                    <div style="padding:5px;font-size:14px; line-height:22px;cursor:pointer;" @click="ShareCurrentDoc($event)">
                        <Icon type="android-share" size="16"></Icon>
                        <span>{{$t('Live.ShareDoc')}}</span>
                    </div>
                    <div style="padding:5px;font-size:14px; line-height:22px;cursor:pointer;" @click="ShareToSyncRoom($event)">
                        <Icon type="share" size="16"></Icon>
                        <span>{{$t('Live.ShareToSyncRoom')}}...</span>
                    </div>
                </div>
            </Poptip>
            <div style="height: 31px;width: 10px;border-right: 1px solid rgb(189, 197, 184);;margin-top:6px;"></div>
            <Tooltip
                :content="$t('Base.Exit')"
                placement="top"
                transfer
            >
                <a class="livePanelTopCtlbtn" id="liveActionEnd" @click="clickClose(true);" style="margin-left: 10px;background:red">
    ...
    >
        <div style="font-size:14px;text-align:left;">
            <div style="font-weight:bold;"><Radio v-model="shareToExistSyncRoom" @on-change="radExistRoomOnChange">Share to an exist SyncRoom</Radio></div>
                ...
                <div style="padding:20px;">
                    ...
                    <span>
                         <Select v-model="ExistSyncRoom_room" style="width:150px">
                            <Option v-for="item in syncroomList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                        </Select>
                    </span>
                </div>
                <div style="text-align:center;"><Button @click="OnShareToExistSyncRoom" type="success" style="width:150px;">OK</Button></div>
            </div>
        </div>
        <div style="text-align:left;margin-top:20px;">
            <div style="font-weight:bold;"><Radio v-model="shareToNewSyncRoom" @on-change="radNewRoomOnChange">Create a new SyncRoom and share to</Radio></div>
             <div style="padding-left:20px;" id="div_shareToNewSyncRoom">
                 ...
                 <div style="padding:5px;">
                     ...
                     <span>
                         <Input v-model="syncroomname" placeholder="Input SyncRoom name..." style="width: 300px"></Input>
                     </span>
                 </div>
                 <div style="padding:5px;">
                     <span style="width:100px;display:inline-block;text-align:right;padding-right:5px;">Contacts</span>
                     <span>
        ...
        <Modal v-model="openLectureModal" :title="$t('PreviewLecture.SyncCourseTemplate')" :closable="true" :mask-closable="false"  width="400" style="z-index:40000;position: absolute;">
            <div style="padding-bottom:20px;font-size:13px;">
              <p style="text-align:center;">
                  <span class="icon icon-alert" style="font-size:56px;color:#0b65fc;"></span>
              </p>
              <p style="margin-top:10px;">{{$t('PreviewLecture.CourseTemplate')}}<q>{{course.Title}}</q>{{$t('PreviewLecture.Updated')}}<span>{{$t('PreviewLecture.DoYouWantSyncCourse')}}</span></p>
              <p style="color:red;margin-top:10px;">{{$t('PreviewLecture.NoticeSyncLecture')}}</p>
            </div>
            <div slot="footer" style="height:35px;">
                    <Button @click="handleSyncCancel" style="float: left;">{{$t('Base.Cancel')}}</Button>
                    <Button type="primary" @click="handleSyncOk" style="float: right">{{$t('PreviewLecture.SyncNow')}}</Button>
            </div>
        </Modal>
  </div>
</template>
  ...
  $("#liveActionTools").click(function(e) {
    ...
    $("#liveActionMinPanelMeeting").click(function(e) {
        if($("#syncRoomInfo").css("display")=="none"){
            $("#previewHolderOuter").removeClass("previewHolderOuter").addClass("previewHolderOuter2");
            $("#previewPanelOperation").removeClass("previewPanelOperation").addClass("previewPanelOperation2");
            ...
            $("#previewHolderOuter").removeClass("previewHolderOuter2").addClass("previewHolderOuter");
            $("#previewPanelOperation").removeClass("previewPanelOperation2").addClass("previewPanelOperation");
  ...
  methods: {
    async Init() {
        ...
        else if(this.getTypeID()=="4")//temp lesson
        {
            ...
            //this.joinMeeting();
            this.thinkMode=true;
        }else if(this.getTypeID()=="5")
        {
            $("#liveAttendeesTip,#liveFilesTip").show();

            this.course = await webapi.getCourseProperty(this.$route.params.courseID);
            
            this.getLectureAttachmentList();
    ...
    async getLectureAttachmentList(){
        let data = await webapi.getCourseLectureAttachmentList(this.getItemID());
        this.showfiles(data.RetData.Attachments);
        // this.showAttendees();
        this.lectureTitle =  data.RetData.Title;
        this.showLectureRoomTitle();
        if(data.RetData.Attachments && data.RetData.Attachments.length>0)
        {
            var res={};
            for(let item of data.RetData.Attachments){
                if(item.FileType === 4){
                    continue;
                }else{
                    res=item;
            ...
            // if(this.course && !this.course.IsTemplate){
            //     let isUpdated = -1;
            //     isUpdated = await webapi.getLectureSyncState(res.ItemID);
            //     if(isUpdated!=1){
    ...
    async handleSyncOk(){
        let res = await webapi.preparationLectureSyncNow(this.getItemID(),this.attachmentItemID);
        if(res){
            PageNumber=1; 
            this.showAttachment(this.attachmentItemID);
            this.openLectureModal = false;
        }
    },
    ...
    showLectureRoomTitle(){
        this.headCourseTitle = this.$t("PreviewLecture.Course");
        this.headTitle = this.$t("PreviewLecture.Lecture");
        this.headLessonTitle=this.lectureTitle;
    ...
    refreshAttachmentsList(callback){
        ...
        webapi.getCourseLectureAttachmentList(this.getItemID()).then(data=>{
            this.showfiles(data.RetData.Attachments);
    ...
    ShowSyncObject(pageid,checkSyncTimeByPage=true) {
        ...
        if(!this.isPause && !this.isPlay)//show lesson line
        {
            webapi.getLecturePageObjectEx(this.getItemID(), this.getAttachmentItemID(this.attachmentID), pageid).then(infoarr => 
            //webapi.getPageObject(this.getItemID(), this.getAttachmentItemID(this.attachmentID), pageid,0).then(infoarr => 
            {
                if (infoarr == null) return;
                for (var txt of infoarr) 
                {
        ...
        else if(this.isPause)//play sync then pause
        {
    ...
    joinMeeting(){
        ...
        var joinmeetingfun=()=>
        {
             websocket.joinmeeting(meetingid,"",2,0,jd => {
                ...
                websocket.updatemeetingdocid(this.attachmentItemID);
                websocket.updateprepareinfo(this.getItemID(),0);
                websocket.onKickMeeting=()=>{
                    
                };               
                websocket.setuploadfilecallback(d => {
                    setTimeout(() => {                
                        webapi.getCourseLectureAttachmentList(this.getItemID(), info => {
                            PageNumber=1;
                            this.showfiles(info.RetData.Attachments);     
                            if (d.createBy + "" == auth.getUserID() + "") {
                                ...
                                {
                                    util.MoveTitle(uploadFileName + " add to KlassRoom success!","/static/images/kloudsync-red.png");
                                    Notification.requestPermission((status)=>
                                    {
                                        if ("granted" == status)
                                        {
                                            ...
                                            var notify = new Notification(
                                                ...
                                                body: uploadFileName + " add to KlassRoom success!"
                                                }
                                            );
                                            notify.onclick = ()=>
                                            {
    ...
    onThinkShow(){
       this.thinkList=[];
       webapi.GetLessonSoundtrackList(this.getItemID(),this.attachmentID).then(list=>{
        if(!list) return;
    ...
    thinkOnCreate(){
        ...
        this.$refs.syncplayer.CreateSync(this.attachmentID,this.getItemID(),this.getAttachmentItemID(this.attachmentID));
    },
    ...
    selectMoreThinkOK(){
        ...
        webapi.AddSoundtrack2Lesson(this.getItemID(),this.selectMoreThink.Selected).then(res=>{
    ...
    onPlayBaseAction(data){
        //console.log("onPlayBaseAction:"+data);
    ...
    ChangeDrawTool4WhiteBoardDiplayChange(isShow){
        if (isShow) {
            $("#liveActionCursor,#liveActionPencil,#liveActionLine,#liveActionHighlighter,#liveActionText,#liveActionNote,#liveActionDraw,#liveActionVideo,#liveActionBoard,#liveActionClear,#liveActionAddPage,#liveActionPlay,#liveActionUndo,#liveActionRedo").hide();
        } else {
            $("#liveActionCursor,#liveActionPencil,#liveActionLine,#liveActionHighlighter,#liveActionText,#liveActionNote,#liveActionDraw,#liveActionVideo,#liveActionBoard,#liveActionClear,#liveActionAddPage,#liveActionPlay,#liveActionUndo,#liveActionRedo").show();
        }
    ...
    thinkOnStartMeeeting(evt){
        ...
        //     this.$router.push({ name: "live", query: { id: res.RetData } });
        // });

        webapi.UpgradeToNormalLesson(this.getItemID()).then(res=>{
            sessionStorage.setItem("ViewDocAutoStartMeeting","1");
            this.$router.push({ name: "live", query: { id: this.getItemID() } });
            ...
            webapi.getTopicMember(this.getItemID()).then(res=>{
                ...
                {                
                    let obj={
                        ...
                        faceurl: user.MemberAvatar == ""? "/static/images/defaultface.png": user.MemberAvatar,
                        isoffline: false,
                        order: 1,
                        showtel: false,
    ...
    async selectFavOK(selectedFav) {
        ...
        if(this.getTypeID()=="4")//temp lesson
        {
            webapi.addFav2Topic(this.getItemID(), selectedFav).then(
                (res) => {
                    this.showfiles(res);
                    if(res.length>0)
                    {
    ...
    async selectSpaceOK(selectedSpaceAttachment){
        ...
        if(this.getTypeID()=="4")//temp lesson
        {
            webapi.addSpace2Topic(this.getItemID(), selectedSpaceAttachment).then(
                (res) => {
                    this.showfiles(res);
                    if(res.length>0)
                    {
    ...
    fileListMenuClick(name, itemid) {
        ...
        {
            if(this.getTypeID()=="5")//temp lesson
            {
                webapi.removeLectureAttachment(itemid).then(res=>{
    ...
    async switchFile(evt, isRightClick) {
        var tar = evt.currentTarget;
        ...
        // if(this.course && !this.course.IsTemplate){
        //     let isUpdated = -1;
        //     isUpdated = await webapi.getLectureSyncState(itemid);
        //     if(isUpdated!=1){
        //         this.openLectureModal = true;
        //     }
        // }
