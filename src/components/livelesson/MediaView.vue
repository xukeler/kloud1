<template>
    <div id="mediaview" style="display:none;">
        ...
        <div style="text-align:center;margin-top:60px;"><img src="/static/images/errormsgtip.svg" /></div>
         <div style="text-align:center;margin-top:10px;color:red;font-size:16px;">Error Connecting</div>
        <div style="text-align:left;padding:20px;font-size:14px;">
           Your microphone might not be properly enabled for this browser. <br />
           <a href="https://kloudsync.peertime.cn/document/57799"  target="_blank">Click here to learn how to enable at your browser.</a><br />
           Then, click Try again button to reconnect again, <span v-show="role!=2">Or you can click Join as audience button to join as a listen only be audience,</span> or click Cancel button to quit.
        </div>
        <div style="height:60px; padding:20px; text-align:center;">
            <Button type="success" @click="tryInitlocalstream" style="float:left;">Try again</Button>
            <Button type="ghost" v-show="role!=2" @click="showErrorTip=false;">Join as audience</Button>
            <Button type="ghost" @click="QuickRoom" style="float:right;">Cancel</Button>
        </div>
        </Modal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async JoinMedia(meetingid,userid,username,role,openVideo,openAudio,callback,retry = 0) 
        {
            ...
            this.reset();
            channel = meetingid;
            client = AgoraRTC.createClient({mode: "live",codec: "vp8",turnServer: {turnServerURL: '23.236.115.138',username: 'test',password: '111111',udpport: '3478',tcpport: '3433',forceturn: false}}); //createClient({ mode: 'interop' });
            uid = userid;
            uname = username;
            urole = role;
            ...
            client.init(AGORA_APPID,() => {
                    client.join(null,"" + channel,parseInt(userid),async(serveruid) => {
                            ...
                            {    
                                ...
                                var trycount=0;
                                while(initresult.Code>1 && trycount<=3)
                                {
        ...
        async tryInitlocalstream()
        {
            ...
            {
                await this.sleep(trycount*500+2000);
                initresult=  await this.initLocalStreamEx(parseInt(uid),this.enableVideo,this.enableAudio);
                //console.log(initresult);
                trycount++;
            }
            if(initresult.Code>0)
                ...
                if (isinstall) {
                    this.getShareScreenRes().then(() => {
                        this.LeaveChannel(screenclient).then(succ => {
                            screenclient = null;
                            screenclient = AgoraRTC.createClient({mode: "live",codec: "vp8",turnServer: {turnServerURL: '23.236.115.138',username: 'test',password: '111111',udpport: '3478',tcpport: '3433',forceturn: false}});
        ...
        initLocalStreamEx(id, openVideo, openAudio) {
            ...
            return new Promise((resolve, reject) => {
                ...
                localStream.init(() => {
                        ...
                        var isDisableAudio=false;
                        var isDisableVideo=false;
                        if (!this.enableAudio && localStream) {
                            localStream.disableAudio();
                            isDisableAudio=true;
                        }
                        if (!this.enableVideo && localStream) {
                            localStream.disableVideo();
                            isDisableVideo=true;
                        }
                        this.setAudioCtlStat(id, this.enableAudio);
                        ...
                        client.on("stream-published", (evt)=> {
                            ...
                            if (!isDisableAudio && !this.enableAudio && localStream) {
                                localStream.disableAudio();
                            }
                            if (!isDisableVideo && !this.enableVideo && localStream) {
                                localStream.disableVideo();
                            }
        ...
        subscribeStreamEvents() {
            ...
            client.on("stream-reconnect-start", evt => {
                //var uid = evt.uid;
                //loger.log("stream-reconnect-start: " + uid);
                //loger.log("Timestamp: " + Date.now());
                //loger.log(evt);
            });
            client.on("disconnect", evt => {
                //var uid = evt.uid;
                //loger.log("disconnect: " + uid);
                //loger.log("Timestamp: " + Date.now());
                //loger.log(evt);
            });
        ...
        createUserContainer(
            ...
            html +=
                '<div style="position:absolute;top:0px; left:0px;z-index:21;background-color:black;color:white;">' +
                username +
                "</div>";
