<template>
    ...
    >
        ...
        <div style="padding: 0px 0px 10px;">
                <Select v-model="selectSpace_Team" style="width:180px" placeholder="请选择团队" @on-change="onSelectSpace_TeamChange($event);">
                <Option v-for="item in selectSpace_TeamList" :value="item.id" :key="item.id">{{ item.name }}</Option>
            </Select>
            <Select v-model="selectSpace_Space" style="width:180px" placeholder="请选择空间" @on-change="onSelectSpace_SpaceChange($event);">
...
export default {
  ...
  methods: {
    ...
    showSpaceAttments(space,page=0,callback){
      this.spaceAttachmentList=[];
      webapi.GetSpaceAttachmentList(space,page,15).then(res=>{
          if(res)
          {
