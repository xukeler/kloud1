<template>
  <div>
    <div id="chat-floating-window" class="chat-floating-window" @mousedown="handleFloatingMouseDown" @mouseup="handleFloatingMouseUp" v-show="showChat && !modalChat">
      <span class="icon icon-chat"></span>
      <div class="messegeNumber" v-show="totalUnreadCount!=0">{{totalUnreadCount}}</div>
    </div>
    <div id="chat-window" class="chat-window" @mousedown="hideEmo($event)" v-show="showChat">
      <div class="chat-window-content">
        <div class="left-part" v-show="showContactList" id="left-part">
          ...
          <div class="card">
            <div class="chat-search-place">
              ...
              <span @click="addNewGroupModel" class="add-Group" style="display:inline-block;" :title="addText"><Icon type="plus" size="12"></Icon></span>
            </div>
            <div class="head-footer-bar">
               <Row>
                  <Col span="8">
                       <div @click="chooseActiveChat($event)" :title="$t('ChatFloating.RecentChat')">
                          <a href="#" v-if="currentMenu !== 'recentChat'"><img src="/static/preview/images/icon-chat.svg" width="35" alt=""></a>
                          <a href="#" v-else><img src="/static/preview/images/icon-chat-active.svg" width="35" alt=""></a>
                       </div>
                       <div class="messegeNumber" v-show="totalUnreadCount!=0">{{totalUnreadCount}}</div>
                  </Col>
                  <Col span="8">
                     <div @click="chooseActiveContact($event)" :title="$t('ChatFloating.AddressBook')">
                         <a href="#" v-if="currentMenu !== 'friendList'"><img src="/static/preview/images/icon-contact.svg" width="35" alt=""></a>
                         <a href="#" v-else><img src="/static/preview/images/icon-contact-active.svg" width="35" alt=""></a>
                     </div>
                  </Col>
                  <Col span="8">
                    <div @click="chooseActiveCourse($event)" :title="$t('ChatFloating.GroupChat')">
                        <a href="#" v-if="currentMenu !== 'courseList'"><img src="/static/preview/images/icon-course.svg" width="35" alt=""></a>
                        <a href="#" v-else><img src="/static/preview/images/icon-course-active.svg" width="35" alt=""></a>
                    </div>
                  </Col>
              </Row>
            </div>
            ...
            <ul>
              <li class="recent-friend" :class="{ bgColor:changeCourseBg == ('Classroom-'+lessonID)}" v-if="mettingClassRoom" @click="chooseCouser('Classroom-'+lessonID,'Current classroom')">
                ...
                <div class="recent-friend-name">
                  <div style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:120px;text-overflow: ellipsis;white-space: nowrap;">{{$t('ChatFloating.CurrentClassroom')}}</div>
                  <div style="margin-top: 3px;color:#33333370;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;">{{$t('ChatFloating.PleaseChat')}}</div>
                </div>
              ...
              <li class="recent-friend" :class="{ bgColor:changeRed == rencentPerson.RongCloudUserID || changeCourseBg== rencentPerson.RongCloudUserID}" v-for="rencentPerson in recentlist" :key="rencentPerson.RongCloudUserID" @contextmenu="rencent_rightClick($event,rencentPerson.RongCloudUserID)" @mousedown="chooseRecent(rencentPerson.ptUserID?rencentPerson.ptUserID:0,rencentPerson.RongCloudUserID,rencentPerson.UserName,rencentPerson.flag)">
                <div class="recent-friend-head"><img :src="rencentPerson.AvatarUrl" width="30" height="30" style="border-radius: 50%"/></div>
                <div class="recent-friend-name">
                  <div style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:120px;text-overflow: ellipsis;white-space: nowrap;">{{rencentPerson.UserName}}</div>
                  <div style="margin-top: 3px;color:#33333370;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;">{{rencentPerson.Content}}</div>
                </div>
                ...
                <div class="delete_rencent" v-show="rencent_chooseFlag==rencentPerson.RongCloudUserID">
                  <ul>
                    <!-- <li>{{$t('ChatFloating.SetTop')}}</li> -->
                    <li @mousedown="delete_oneRencent(rencent_chooseFlag,$event)">{{$t('Common.Delete')}}</li>
                  </ul>
                </div>
              </li>
            </ul>
          ...
          <div class="friend-list" v-show="currentMenu === 'friendList' && !showSearch">
            <ul v-if="mettingClassRoom">
              <li class="friend" :class="{ bgColor:changeCourseBg == ('Classroom-'+lessonID)}" @click="chooseCouser('Classroom-'+lessonID,'Current classroom')">
                <div class="friend-head">
                   <img src="/static/preview/images/group-default.svg" width="30" height="30" style="border-radius: 50%"/>
                </div>
                <div class="user-name"><div style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;">{{$t('ChatFloating.CurrentClassroom')}}</div></div>
                <div style="clear: both;"></div>
              </li>
            </ul>
            <ul>
              <li class="friend" :class="{ bgColor:changeCourseBg == 'PeerBot'}" @click="chooseBot('PeerBot')">
                <div class="friend-head">
                   <img src="/static/preview/images/PeerBot.png" width="30" height="30" style="border-radius: 50%"/>
                </div>
                <div class="user-name"><div style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:110px;text-overflow: ellipsis;white-space: nowrap;">{{botName}}</div></div>
                <div style="clear: both;"></div>
              </li>
            </ul>
            <ul v-for="users in friendList " :key="users.Index">
              <li style="padding:0px 10px;">{{users.Index}}</li>
              <li class="friend" :class="{ bgColor:changeRed == friend.RongCloudUserID}" v-for="friend in users.Users" :key="friend.RongCloudUserID" @mousedown="chooseFriend(friend.UserID,friend.RongCloudUserID,friend.Name)">
                <div class="friend-head">
                   <img v-if="friend.AvatarUrl==''" src="../../../static/images/user.svg" width="30" height="30" style="border-radius: 50%"/>
                   <img v-else  :src="friend.AvatarUrl" width="30" height="30" style="border-radius: 50%"/>
                </div>
                <div class="user-name"><p style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;">{{friend.Name}}</p></div>
                <div style="clear: both;"></div>
              </li>
            </ul>
          </div>
          <div class="search-friend-list" v-show="currentMenu === 'friendList' && showSearch">
              <ul>
                <li class="search-friend" :class="{ bgColor:changeRed == searchUser.RongCloudUserID}" v-for="searchUser in searchflist" :key="searchUser.RongCloudUserID" @mousedown="chooseFriend(searchUser.UserID,searchUser.RongCloudUserID,searchUser.Name)">
                  <div class="search-friend-head">
                    <span v-if="searchUser.AvatarUrl==''"><img  src="../../../static/images/user.svg" width="30" height="30" style="border-radius: 50%"/></span>
                    <span v-else><img  :src="searchUser.AvatarUrl" width="30" height="30" style="border-radius: 50%"/></span>
                  </div>
                  <div class="search-friend-name">
                    <div style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;">{{searchUser.Name}}</div>
                    <div style="margin-top: 3px;color:#33333370;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;">Peertime ID:NULL</div>
                  </div>
                  <div style="clear: both;"></div>
                </li>
              </ul>
          </div>
          <div class="search-course-list" v-show="currentMenu === 'courseList' && showSearch">
              <ul>
                <li class="course" :class="{ bgColor:changeCourseBg == couser.GroupID}" v-for="couser in seachCouserList" :key="couser.GroupID" @mousedown="chooseCouser(couser.GroupID,couser.GroupName)">
                  <div class="course-head"><img src="/static/preview/images/group-default.svg" width="30" height="30" style="border-radius: 50%"/></div>
                  <div class="course-name">
                    <p style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;padding-top:9px;">{{couser.GroupName}}</p>
                  </div>
                  <div style="clear: both;"></div>
                </li>
              </ul>
          </div>
          <div class="course-list" v-show="currentMenu === 'courseList' && !showSearch">
            <div class="course-list-people" id="courseListPeople">
              <ul> 
                  <li class="course" :class="{ bgColor:changeCourseBg == ('Classroom-'+lessonID)}" v-if="mettingClassRoom" @click="chooseCouser('Classroom-'+lessonID,'Current classroom')">
                    <div class="course-head"><img src="/static/preview/images/group-default.svg" width="30" height="30" style="border-radius: 50%"/></div>
                    <div class="course-name">
                      <p style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;padding-top:9px;">{{$t('ChatFloating.CurrentClassroom')}}</p>
                    </div>
                  </li>
                  <li class="course" :class="{ bgColor:changeCourseBg == couser.GroupID}" v-for="couser in couserList" :key="couser.GroupID" @mousedown="chooseCouser(couser.GroupID,couser.GroupName)">
                    <div class="course-head"><img src="/static/preview/images/group-default.svg" width="30" height="30" style="border-radius: 50%"/></div>
                    <div class="course-name">
                      <p style="font-size: 13px;font-weight: bold;overflow: hidden; max-width:130px;text-overflow: ellipsis;white-space: nowrap;padding-top:9px;">{{couser.GroupName}}</p>
                    </div>
        ...
        <div class="right-part" id="right-part">
          ...
          <div class="toolbar-header-part">
             <span :title="$t('ChatFloating.ClosedOrPullsidebar')" @click="showLeftPart" style="float:left;cursor: pointer;"><img :class="{rotatePic:isRotate}" src="/static/preview/images/icon-menu.svg" width="14" alt=""></span>
             <span class="chat-groupName" style="margin-left:10px;float:left;">{{groupName}}
             </span>
             <span :title="$t('ChatFloating.GroupMembers')" v-show="showGroup" @click="showDropdownItem($event)"  style="margin-left:5px;float:left;cursor: pointer;">
               <Icon  type="chevron-down" size="11" v-show="!showDropdown"></Icon>
               <Icon  type="chevron-up" size="11" v-show="showDropdown"></Icon>
            </span>
             <span style="margin-left:20px;float:left;"></span>
             <span :title="$t('ChatFloating.Close')" @click="handleCloseChat" style="float:right;margin-right:10px;cursor: pointer;"><img src="/static/preview/images/icon-close.svg" width="12" alt=""></span>
             <span style="clear: both;"></span>
             <!-- <span style="float:right;margin-right:12px;"><img src="/static/preview/images/icon-video.svg" width="15" alt=""></span>
             <span style="float:right;margin-right:12px;"><img src="/static/preview/images/icon-call.svg" width="12" alt=""></span> -->
          </div>
          <div class="content-part" id="content-part" v-show="showContent">
                ...
                <li  ref="messageList" v-for="item in messageList">
                  ...
                  <div class="main" :class="{self:item.IsMy}" v-if="!item.ObjectName">
                    ...
                    <div class="text messageRecord">
                      <p class="line" v-if="item.shareDocUrl !== undefined" @click="goToshareLive(item.shareDocUrl)" style="width:180px;cursor: pointer;position: relative;">
                          <Row>
                            <Col span="24">
                              <Row>
          ...
          <div class="footer-part" v-if="showContent" @click.stop="getFocus">
              ...
              <div style="float:left;margin-left:12px;padding:10px 0px;position: relative;cursor: pointer;" :title="$t('ChatFloating.Attachment')">
                <img src="/static/preview/images/icon-attach.svg" width="14" height="14" alt="">
                <div class="chat-upload">
                  <input id="chatUpload"  type="file" @change="uploadInputChange(this);"  accept="video/mp4,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-powerpoint,application/pdf, application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
                </div>
              </div>
              <!--收藏按钮-->
              ...
              <div style="clear: both;"></div>
              <div v-show="showemojiList" id="rongcloudview-emoji" style="position:absolute;bottom:115px; right:0px;width:260px;background-color:#ccc;padding:5px 2px;border: 0.5px solid #888888;">
    ...
    <Modal :title="$t('ChatFloating.AlertAddParticipantsTitle')"
           ...
           <div class="group-content" id="user-checked">
             <ul v-for="users in friendList " :key="users.Index">
              <li style="padding:0px 16px;">{{users.Index}}</li>
              <li class="friend" v-for="friend in users.Users" :key="friend.RongCloudUserID">
                <div class="user-checked"><input type="checkbox" name="friendId" :id="friend.RongCloudUserID" :friendName="friend.Name"/></div>
                <div class="friend-head">
          ...
          <Row>
            ...
            <Col span="8">
              <span class="share-pciture">
                  <img id="sharePicImage" src="../../../static/images/upload-pic.png" width="80" height="80" alt=""/>
                  <input id="sharePic"  type="file" @change="changeSharePic();"  accept="image/jpeg,image/png"/>
              </span>
            </Col>
          </Row>
        </div>
    ...
    <audio id="peerChatAudio"  autoplay></audio>
  </div>
</template>
...
import speakBotContent from "./speakBotContent.js";
...
export default {
  ...
  watch: {
      ...
      if (newVal != oldVal) {
        if (newVal) {
          this.$nextTick(() => {
            ...
            let self = this;
            ////只能单设备登陆的情况
            // webapi.getOnlineStatus().then((retdata)=>{
            //   //RetData 1 -- Online 0 -- Offline
            //   if(retdata==0){
            //     setTimeout(() => {
            //       this.initWebapi().then((res)=>{
            //         if(res==0){
            //           return;
            //         }
            //         //监听网络状态，重连融云
          ...
          util.LoadKendoFiles().then(()=>{
            ...
            if($("#chat-window").data("kendoWindow")){
              $("#chat-window").data("kendoWindow").close();
            }
          })
        }
      }
    ...
    lessonID(newVal, oldVal) {
      if (newVal != oldVal) {
        if (newVal != "" && this.classRoomRouteName == "live") {
          this.$nextTick(function() {
            this.mettingClassRoom = true;
            let str = "Current classroom";
            let islessonID = "Classroom-" + newVal;
            this.JoinMeeting(islessonID, str);
            console.warn("课程lesson",islessonID);
          });
        } else {
          this.quitMetting(-1,this.RongCloudUserID,"");
            ...
            this.$nextTick(function(){
              ...
              this.reconnectInit().then((res)=>{
                if(res == 0){
                  ...
                  self.liveLessonShareSelect=true;
                  self.liveShareFile.attachmentId=obj.attachmentId;
                  self.liveShareFile.attachmentName=obj.attachmentName;
    ...
    //监测路由
    $route(to,from){
      if(to.name=="live" || to.name=="doc" || to.name=="view"){

        // setTimeout(() => {
        //   util.LoadJsFiles(["../../../static/RongcloudJs/RongIMLib-2.4.0.js","../../../static/RongcloudJs/RongEmoji-2.2.7.js"]).then(()=>{
        //     //进入会议,重新初始化连接
        //     if(RongIMClient.getInstance()!=null && this.Connected=="CONNECTED"){
        //       // RongIMClient.getInstance().disconnect();
        //       // this.initWebapi();
        //     }else if(RongIMClient.getInstance()!=null && this.Connected!="CONNECTED"){
        //       if(!IsKloudSync()){
        //         this.initWebapi();
        //       }
        //     }
      ...
      //手机端不出现chat
      if(to.name=="attachment" || to.name=="document"){
        util.LoadKendoFiles().then(()=>{
          this.modalChat = false;
          $("#chat-window").css("display","none");
          if($("#chat-window").data("kendoWindow")){
  ...
  computed: {
    ...
    addText(){
      switch (this.currentMenu) {
        case 'recentChat':
          return  this.$t('ChatFloating.CreateDiscussionGroup');
        case 'friendList':
          return  this.$t('KloudSyncAccountContact.AddMyContact');
        default:
          break;
      }
    }
  ...
  mounted() {
    ...
    this.peerChatAudio = document.getElementById("peerChatAudio");
    let self = this;
    if(this.showChat)
    {
      this.loadKendoEle();
  ...
  methods: {
    ...
    loadKendoEle(){
      let self = this;
      util.LoadKendoFiles().then(()=>{
        $("#chat-floating-window").kendoDraggable({
        dragstart: function(e) {
          let offset = $("#chat-floating-window").offset();
          self.dragX = e.x.location - offset.left;
          self.dragY = e.y.location - offset.top;
        },
        drag: function(e) {
          let left = e.x.location - self.dragX;
          let top = e.y.location - self.dragY;
          $("#chat-floating-window").css({left: left,top: top,right: "auto",bottom: "auto"});
        }
        });
        $("#chat-window").kendoWindow({
          title: false,
          visible: false,
          width: "500",
          height: "500",
          ...
          resize: function() {
            let width = $("#chat-window").parent().width();
            let height = $("#chat-window").parent().height();
            let messageWidth = $("#right-part").width();
            let max = messageWidth * 0.62;
            $(".messageRecord").css("max-width", max);
            if ($("#left-part").css("display") == "none") {
              if (width < 300) {
                $("#chat-window").parent().css("width", 301);
              }
            } else {
              if (width < 500) {
                $("#chat-window").parent().css("width", 501);
              }
            }
            if (height < 400) {
              $("#chat-window").parent().css("height", 400);
              $("#chat-window").parent().css("min-height", 400);
            }
          }
        });
        $("#chat-window").parent().css({"min-width":501,"z-index":1000});
        $("#chat-window").kendoDraggable({
          filter: ".dragArea, .toolbar-header-part",
          dragstart: function(e) {
            let offset = $("#chat-window").parent().offset();
            self.dragX = e.x.location - offset.left;
            self.dragY = e.y.location - offset.top;
          },
          drag: function(e) {
            let left = e.x.location - self.dragX;
            let top = e.y.location - self.dragY;
            $("#chat-window").parent().css({ left: left, top: top, right: "auto", bottom: "auto" });
          }
        });
      });
    },
    ...
    changeSharePic(){
      let self = this;
      var shareFile = document.getElementById("sharePic").files[0];
      let reader=new FileReader();
      reader.onload=function(e){
        let dataUrl=this.result;
        $("#sharePicImage").attr("src",dataUrl);
      }
    ...
    shareCancel(){
      ...
      this.changeShareDoc(shareFile);
      $("#sharePic").val("");
      $("#sharePicImage").attr("src","../../../static/images/upload-pic.png");
    },
    shareOk(){
      let self = this;
      this.handleFloatingClick().then((res)=>{
        if(res){
          self.chooseActiveContact();
          self.shareToSendBefore();
          webapi.GetShareDocumentKey(self.liveShareFile.attachmentId,self.shareToUserId,self.liveShareFile.attachmentName).then((result)=>{
            self.shareDocModal=false;
            self.selectFromCollection=false;
            let href=window.location.href.split("//")[0]+window.location.href.split("//")[1].split("/")[0];
            let shareDocThumbnailUrl=self.liveShareFile.docUrl;//自定义上传图片的缩略图Url,默认文档第一页
            let shareDocAvatarUrl=self.myHead;//分享人头像
            let shareDocUsername=self.currentUser;//分享人的姓名
            let shareDocTime=self.shareTime;//分享时间
            let attachmentID=self.liveShareFile.attachmentId;//文档的attachmentID
            self.SendShareDocMessage(self.shareTitle,href+"/attachment/"+attachmentID,shareDocThumbnailUrl,shareDocAvatarUrl,shareDocUsername,shareDocTime,attachmentID);
            let shareFile={attachmentId:"",attachmentName:"",attachmentUrl:""};
            self.changeShareDoc(shareFile);
          });
        }
      })
    },
    ...
    ShareToFriend(ptUserId,id, groupName,avatarUrl){
      ...
      $.ajax({
        ...
        data: "",
        success: function() {
          $("#sharePicImage").attr("src",ajaxUrl);
        },
        error: function() {
        }
      });
    ...
    shareToSendBefore(){
      ...
      this.groupName = this.sharePeople.peopleName;
      this.changeRed = this.sharePeople.RongCloudUserID;
      this.messageList = [];
      this.RongCloudUserID = this.sharePeople.RongCloudUserID;
      this.shareToUserId = this.sharePeople.ptUserId;//私人共享文件

      let self = this;
      this.showHistoryMessage2(self.sharePeople.RongCloudUserID,'PRIVATE').then((res)=>{
        if(res==0){
          return;
        }
      ...
      this.clearPrivateMessageCount(this.sharePeople.RongCloudUserID);
    ...
    //鼠标右击事件
    rencent_rightClick(e,id){
      this.rencent_chooseFlag=id+"";
      // let X=e.offsetX+'px';
      // let Y=e.offsetY+'px';
      // $(".delete_rencent").css({left:X,top:Y});
      e.preventDefault();//阻止浏览器默认右击事件;
    },
    //点击录音按钮开始录音
    startRecord() {
    ...
    //播放当前录制的音频
    playRecording() {
      this.recorder.play(this.peerChatAudio);
    },
    //取消录音
    cancelAudioMsgSend() {
      clearInterval(this.speakTime);
    ...
    sendAudioMsg() {
      ...
      // this.recorder.play(this.peerChatAudio); 播放当前录音
    },
    goToTheRoom(msg) {
      //判断是否是进入房间的消息
      ...
      if (joinRoomReg.test(msg)) {
        ...
        //进入房间
        this.$router.push({ name: "live", query: { id: "-" + roomNum } });
        return true;
      } else {
        return false;
      }
    ...
    scrollTop() {
      this.$nextTick(function() {
        document.getElementById("courseListPeople").scrollTop = 0;
      });
    },
    ...
    JoinMeeting(couserId, couserName) {
      this.flag = "GROUP";
      this.changeRed = -1;
      ...
      this.showContent = true;
      this.showGroup = true;
      let width = parseInt($("#left-part").width());
      let allWidth = parseInt(
        $("#chat-window").parent().width()
      );
      if (this.showContactList == true) {
        this.showContactList = false;
        this.isRotate = false;
        $("#chat-window").parent().css("width", allWidth - width - 1 + "px");
        $("#chat-window").parent().css("min-width", 301);
      }
      $("#chat-window").parent().css({ right: "2px", bottom: "2px", left: "auto", top: "auto" });
      $("#chat-floating-window").css({left: "auto",top: "auto",right: "24px",bottom: "19px"});
    },
    quitMetting(couserId, quitRongCloudUserID,couserName) {
      this.flag = "GROUP";
      this.changeRed = -1;
      this.changeCourseBg = -1;
      this.groupName = couserName;
      ...
      this.showContent = false;
      if(this.Connected=="CONNECTED"){
        this.removeLocalConversation(quitRongCloudUserID,this.flag);
      }
    },
    removeLocalConversation(id,flag){
      var targetId = id+"";
      let self = this;
      //融云删除会话
      var conversationType;
    ...
    addGroupCancel() {
      this.addNewGroup = false;
      $("#user-checked").find("input[name='friendId']:checkbox").each(function() {
          $(this).prop("checked", false);
      });
    },
    addNewGroupModel() {
      if(this.Connected != 'CONNECTED'){
        window.alert("Not connected to chat sever, please wait patiently");
        return
      }

      if (this.currentMenu === "recentChat") {
        this.addNewGroup = true;
      }else if(this.currentMenu === "friendList"){
        this.$refs.addChatFriendModal.handInviteNewFriends();
      }else{
        this.$Message.warning({content:this.$t('ChatFloating.AlertPleaseSelectDiscussionGroup'),duration: 3});
      }
    },
    updatedMessage() {
      this.$nextTick(function() {
        var div = document.getElementById("content-part");
    ...
    chooseActiveContact(e) {
      ...
      friends.getFriendsSortList(function(friends) {
        let result = friends;
    ...
    showLeftPart() {
      ...
      if (this.showContactList == true) {
        this.showContactList = false;
        this.isRotate = false;
        $("#chat-window").parent().css("left", left + width + 1 + "px");
        $("#chat-window").parent().css("width", allWidth - width - 1 + "px");
        $("#chat-window").parent().css("min-width", 301);
      } else {
        this.showContactList = true;
        this.isRotate = true;
        $("#chat-window").parent().css("left", left - width - 1 + "px");
        $("#chat-window").parent().css("width", allWidth + width + 1 + "px"); //border-left为1px
        $("#chat-window").parent().css("min-width", 501);
      }
    },
    init() {
      util.LoadJsFiles(["../../../static/RongcloudJs/RongIMLib-2.4.0.js","../../../static/RongcloudJs/RongEmoji-2.2.7.js"]).then(()=>{
    ...
    replayMsgToUserPost(id,textT){//post message to bot
      let sendObj={
          "type": "message",
          "from": {
            id: "user1"
      ...
      $.ajax({
        url:"https://directline.botframework.com/v3/directline/conversations/"+id+"/activities",
        type: "post",
        ...
        success: res => {
          this.replayMsgToUserGet(id,parseInt(res.id.split("|")[1]));
        },
      });
    },
    replayMsgToUserGet(cid,wid) {//get reponse from bot
      $.ajax({
        url:"https://directline.botframework.com/v3/directline/conversations/"+cid+"/activities?watermark="+wid,
        type: "get",
        beforeSend: function(xhr) {
          xhr.setRequestHeader(
            "Authorization",
        ...
        success: res => {
          ...
          if (res.activities.length==0) {
            window.setTimeout(() => {
              this.replayMsgToUserGet(cid,wid);
            }, 2000);
          ...
          for (let [i, v] of res.activities.entries()) {
            if (parseInt(v.replyToId.split("|")[1]) == wid ){
              let str="";
    ...
    sendMsgToBot(sendMsg) {
      ...
      $.ajax({
        ...
        //data: JSON.stringify(sendObj),
        success: res => {
          this.replayMsgToUserPost(res.conversationId,sendMsg);
        }
      });
    },
    insertEmoji(evt) {
    ...
    onReceiveMessage(message) {
      ...
      if (message.conversationType == 1) {
        ...
        } else if (groupId.indexOf("Classroom-") != -1 && this.lessonID == 0) {
    ...
    uploadInputChange() {
      let self = this;
      var inputFile = document.getElementById("chatUpload").files[0];
      if (inputFile) {
    ...
    ...mapMutations(['changeConnected',"changeShareDoc"])
  }
};
</script>
