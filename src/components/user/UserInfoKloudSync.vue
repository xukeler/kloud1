  ...
  <div style="display:none; width:120px;" id="usersetmenu" class="usersetmenu" @mouseover="usermouseOver();" @mouseout="usermouseOut();">
    ...
    <Modal
      ...
      flex-direction: row;">
        ...
        <div style="flex:1 1 120px">There are {{appplyCompanie.length}} more organization invited you to join,do you want to confirm the invitation now</div>
      </div>
    ...
    methods: {
              ...
              success: function (resultString) {
                ...
                if (result.RetCode === 0){
                  if(result.RetData && self.uploadCompanyFile){
                    self.uploadCreateCompanyUrl = GetAPIUrl() + 'School/UploadCompanyAvatar?companyID='+result.RetData;
      ...
      initOrganizationList(){
        ...
        this._getData(url, null, (list) => {
          ...
          //将My school
          for(let item of this.allLisOrganizationList){
      ...
      async joinLesson()
      {
        ...
        this.checkClassRoomExist(""+this.joinLessonID).then((res) =>
        {
          ...
          else
          {
            ...
            //this.$router.push({ name: 'live', query: { incidentid: this.joinLessonID, type: 1 } });
            //this.$router.push({ name: 'live', query: { incidentid: "-" + this.joinLessonID } });
            //this.$router.push('/live/' + "-" + this.joinLessonID);
            ...
            this.$router.push({ name: 'live', query: { id: "" + this.joinLessonID } });
            return;
            //this.getClassRoomLessonID(this.joinLessonID).then(lessonid =>
            //{
            //  if (lessonid < 1)
            //  {
            //    this.changeLessonID(0);
      ...
      joinLessonEx()
      {
        ...
        this.getMeetingID(this.joinLessonID).then((res) =>
        {
          ...
          {
            this.showNoticeInfo = false;
            this.$router.push({ name: 'live', query: { incidentid: res } });
          }
        });
      },
      enterKlassroom()
      {
        ...
        {
          alert("No ClassRoomID, Please set your ClassRoomID first!");
          return;
        }
        this.changeLessonID(0);
        //this.$router.push({ name: 'live', query: { incidentid: classRoomID, type: 1 } });
        //this.$router.push({ name: 'live', query: { incidentid: classRoomID } });
        ...
        this.$router.push({ name: 'live', query: { id: classRoomID } });
        return;
      ...
      lessonOK()
      {
        ...
        this.$router.push({ name: 'live', query: { id: this.selectLessonID } });
      },
      lessonCancel()
      {
        this.startLessonDlgEx = false;
      ...
      clickNoticeTitle(evt)
      {
        ...
        this.$router.push({ name: 'live', query: { id: lessonid } });
        this.showNoticeInfo = false;
      },
      getLessonsInfo(ids)
      {
